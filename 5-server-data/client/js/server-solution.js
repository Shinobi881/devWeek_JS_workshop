...
//A factory method
function getAnimalsFromServer(callback){
 $.ajax({
  url: "http://localhost:3030/animals"
}).done(function(data) {
  callback(data);
...
$(document).ready(function(){
  ...
  //call the factory method to get animals from server
  //Can you refactor this into an object? Hint: you will 
  //need bind. 

  getAnimalsFromServer(function(data){
   data.forEach(function(animal){
     animalData.push(new Animal(animal.name, animal.species, animal.key));
   })
  })
   var game=new Controller(animalData,view, 9);
});
